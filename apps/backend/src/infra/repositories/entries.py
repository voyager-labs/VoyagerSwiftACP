...
class EntriesRepository:
    ...
    def get_by_id(self, id_: int) -> Optional[EntrySchema]:
        """기본 키(EntrySchema.id)로 단건 조회"""

        stmt = select(EntrySchema).where(EntrySchema.id == id_)

        return self.session.exec(stmt).first()

    def get_by_path(self, path: str) -> Optional[EntrySchema]:
        """고유 경로 컬럼(EntrySchema.path)로 단건 조회"""

        stmt = select(EntrySchema).where(EntrySchema.path == path)

        return self.session.exec(stmt).first()

    def delete_by_id(self, id_: int) -> int:
        """기본 키 컬럼(EntrySchema.id)로 단건 삭제"""
    ...
    def delete_by_path(self, path: str) -> int:
        """고유 경로 컬럼(EntrySchema.path)로 단건 삭제"""
    ...
    def update_by_id(self, id_: int, fields: Mapping[str, Any]) -> Optional[EntrySchema]:
        """기본 키로 일부 필드를 갱신하고 갱신된 레코드를 반환합니다.

        동작
        - 대상이 없으면 None 반환(예외 없음).
        - 전달된 키 중 스키마(EntrySchema)에 존재하는 속성만 반영합니다. `id` 키는 무시됩니다.
        - 세션 커밋은 외부 컨텍스트에 위임하며, 본 메서드는 flush만 수행합니다.

        참고
        - 허용되는 키 셋은 런타임에 `EntrySchema.model_fields`에서 추출하여 사용합니다
        (스키마 정의 변경 시 자동 동기화)
        """
        ...
        allowed_keys = EntrySchema.model_fields.keys()
    ...
    def upsert_one(self, item: EntrySchema) -> EntrySchema:
        ...
        result = self.session.exec(cast(SelectOfScalar[EntrySchema], stmt))
        row = result.one()
        self.session.flush()
        return row

    def batch_upsert(self, items: Sequence[EntrySchema]) -> list[EntrySchema]:
        ...
        by_path: dict[str, EntrySchema] = {r.path: r for r in all_rows}
