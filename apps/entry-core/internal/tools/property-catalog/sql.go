...
func renderDefinition(row DefinitionRow) string {
	...
	builder.WriteString("    definition_revision, lifecycle_state, seed_owner, seed_version,\n")
...
func renderTerm(row TermRow) string {
	...
	builder.WriteString("    workspace_id, property_id, term_kind, ordinal, term_value, seed_owner,\n")
	builder.WriteString("    seed_version, seed_source_version\n")
	builder.WriteString(")\nSELECT\n")
	builder.WriteString("    workspace_id,\n")
	builder.WriteString("    X'" + blobHex(row.PropertyID) + "',\n")
