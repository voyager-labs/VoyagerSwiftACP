...
func TestWorkspaceIDTimestampField(t *testing.T) {
	id, err := NewWorkspaceID()
	if err != nil {
		t.Fatalf("NewWorkspaceID() error = %v", err)
	}

	// The 48-bit unix-millisecond timestamp must occupy bytes 0-5 (RFC 9562).
	got := uint64(id[0])<<40 | uint64(id[1])<<32 | uint64(id[2])<<24 |
		uint64(id[3])<<16 | uint64(id[4])<<8 | uint64(id[5])

	if got == 0 {
		t.Fatalf("timestamp field (bytes 0-5) = 0, want non-zero current unix-millis")
	}
	want := time.Now().UnixMilli()
	if diff := int64(got) - want; diff < -5000 || diff > 5000 {
		t.Fatalf("timestamp field = %d ms, want within 5000ms of now (%d ms)", got, want)
	}
}
