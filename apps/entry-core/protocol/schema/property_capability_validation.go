...
// derivation을 공유한다. origin은 lifecycle과 무관하게 두 canonical 값 중
// 하나여야 하고, unsupported capability도 definition lifecycle, origin 및
// value contract에서 유도되는 사유와 정확히 일치해야 한다 — 특히
...
func validConditionCapabilityForDefinition(definition PropertyDefinition) bool {
	...
	if origin != domainentry.PropertyOriginBuiltIn && origin != domainentry.PropertyOriginUserDefined {
		return false
	}
