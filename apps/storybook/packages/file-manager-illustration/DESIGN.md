...
- **Item scale decision:** the screenshot renders the item larger than proportionate, so a modest, uniform scale-down is applied rather than touching only one element: icon 14→13px, label 12→11px regular, chevron 10→9px, inner component height 20→18px, keeping the 4px element gap, 0px icon–label gap, and single-line truncation. The icon keeps its 2px lead over the label (13 vs 11) to preserve the native hierarchy. This stays close to the native `PathBreadcrumbView` (14/12/10/20) while making the isolated item read compact.
