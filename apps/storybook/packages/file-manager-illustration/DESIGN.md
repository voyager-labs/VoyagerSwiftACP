...
- **Item geometry:** follows the native `PathBreadcrumbView` contract exactly — 14px icon box, 12px regular label, 10px chevron glyph, 4px element gap, 0px icon–label gap, 20px inner component height, single-line truncation with `line-height: 1`. The earlier screenshot-driven 13/11/9/18 scale-down was reverted once the oversized rendering was traced to the missing `line-height` constraint, not the font sizes.
