...
final class ONB004ConfigureAIProviderDuringOnboardingTests: XCTestCase {
    ...
    func testSetUpLaterCancelsInFlightConnectionAndIgnoresLateTerminal() async {
        ...
        let continuation = LockIsolated<AsyncThrowingStream<CodexProviderLoginState, Error>.Continuation?>(nil)
        ...
        await store.receive(\.aiProviderSetup.row[id: provider].startProviderLogin) { state in
