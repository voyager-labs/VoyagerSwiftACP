...
extension FileManagerWindowCommandRoutingReducer {
    ...
    private func restoreFailureReason(for snapshot: ClosedContentTabSnapshot, state _: State) -> RestoreFailureReason? {
        ...
        case let .collectionFile(url):
            return fileManagerClient.fileExistsWithIsDirectory(url.path, nil) ? nil : .missingCollection
        case .aiChat:
    ...
    private func restoreFailureFeedbackEffect(_ reason: RestoreFailureReason) -> Effect<Action> {
        ...
        return .run { _ in
            await collectionAlertClient.showCollectionOpenErrorAlert("Cannot Restore Tab", message)
        }
    }
    ...
    func handleRequestedCommand(_ command: Action.WindowCommand, state: inout State) -> Effect<Action> {
        guard !isCommandBlockedByPendingPin(command, state: state),
              !isCommandBlockedByPendingClose(command, state: state)
        else { return .none }
        if let effect = handleContentTabCommandIfNeeded(command, state: &state) { return effect }
        if let effect = handleEntryCommandIfNeeded(command, state: &state) { return effect }
        if let effect = handleComposerCommandIfNeeded(command, state: &state) { return effect }
        if let effect = handleNavigationCommandIfNeeded(command, state: state) { return effect }
        if let effect = handleLayoutCommandIfNeeded(command, state: &state) { return effect }
        if let effect = handleUndoCommandIfNeeded(command, state: &state) { return effect }
        if let effect = handleDuplicateCommandIfNeeded(command, state: &state) { return effect }
        return .none
    }

    func isCommandBlockedByPendingPin(_ command: Action.WindowCommand, state: State) -> Bool {
        state.pendingSelectedContentTabPinMutation != nil && isPinToggleCommand(command)
    }

    func isPinToggleCommand(_ command: Action.WindowCommand) -> Bool {
        if case .toggleActiveContentTabPin = command { return true }
        return false
    }

    func isCommandBlockedByPendingClose(_ command: Action.WindowCommand, state: State) -> Bool {
        state.pendingSelectedContentTabClose != nil && isCloseBlockedCommand(command)
    }

    func isCloseBlockedCommand(_ command: Action.WindowCommand) -> Bool {
        switch command {
        case .openNewContentTab, .closeActiveContentTab, .closeSelectedContentTabs,
             .toggleActiveContentTabPin, .restoreLastClosedContentTab, .duplicateContentTab,
             .duplicateActiveContentTab, .duplicateSelectedContentTabs, .saveCollection, .saveCollectionAs:
            true
        default:
            false
        }
    ...
    func handleContentTabCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: inout State,
    ) -> Effect<Action>? {
        switch command {
        case .openNewContentTab, .selectContentTab:
            handleContentTabCommand(command, state: state)
        case .closeActiveContentTab:
            state.contentTabs.activeTabID.map { .send(.closeContentTabRequested($0)) } ?? .none
    ...
    func handleEntryCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: inout State,
    ) -> Effect<Action>? {
        switch command {
        case .newFolder, .openSelectedItem, .quickLookSelectedItem, .cut, .copy, .paste, .duplicate,
             .makeAlias, .selectAll, .copyAbsolutePaths, .copyURLs:
    ...
    func handleComposerCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: inout State,
    ) -> Effect<Action>? {
        switch command {
        case .saveCollection, .saveCollectionAs, .find, .toggleComposer, .newChat, .showChatHistory:
    ...
    func handleNavigationCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: State,
    ) -> Effect<Action>? {
        switch command {
        case .goBack, .goForward, .goToEnclosingDirectory:
    ...
    func handleLayoutCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: inout State,
    ) -> Effect<Action>? {
        switch command {
        case .toggleSidebar, .setViewLayout, .setGroupKey, .setSortKey, .setSortOrder:
    ...
    func handleUndoCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: inout State,
    ) -> Effect<Action>? {
        switch command {
        case .requestUndo, .requestRedo:
    ...
    func handleDuplicateCommandIfNeeded(
        _ command: Action.WindowCommand,
        state: inout State,
    ) -> Effect<Action>? {
        switch command {
