...
struct FileManagerContentSyncReducer {
    ...
    /// 후속 modification이 한 delivery로 합쳐지면 ItemRenamed|ItemModified를 함께
    /// 보고한다. 이 경우 독립 수정 신호(ItemModified)를 보존해야 하므로 순수 rename
    ...
    private func isCommandIdentityEcho(_ event: FileChangeGatewayEvent) -> Bool {
        ...
        let independentlyModified = event.flags & UInt32(kFSEventStreamEventFlagItemModified) != 0
        return renamed && !independentlyModified
    }
