...
private extension ContentTabSidebarButton {
    ...
    func movement(from origin: NSPoint, to location: NSPoint) -> CGFloat {
        hypot(location.x - origin.x, location.y - origin.y)
    }

    func cleanupDragging() {
    ...
    @objc
    func performTrailingAction() {
        guard trailingActionButton.isEnabled else { return }
        onTrailingAction()
    }

    @objc
    func moveToWindow(_ sender: NSMenuItem) {
        guard !isMovePending,
