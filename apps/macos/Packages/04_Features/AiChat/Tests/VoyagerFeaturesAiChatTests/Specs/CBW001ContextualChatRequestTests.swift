...
final class CBW001ContextualChatRequestTests: XCTestCase {
    ...
    /// CBW-001-show_request_processing_state: composer와 selector, session search는 기존 VoyagerDS 토큰을 사용한다.
    /// VOY-635 named surface에 남은 radius, selected fill, popover shadow literal이 제거되는지 검증합니다.
    /// - 검증 내용: input, model/thinking selector, sessions source의 semantic token과 금지 literal을 확인합니다.
    /// - 사전 조건: 각 UI가 VoyagerShared를 import하고 colorScheme을 제공합니다.
    /// - 기대 결과: composer·popover·selected row·search field가 새 토큰 없이 기존 토큰으로 표현됩니다.
    func testShowRequestProcessingStateUsesVoyagerDSTokensForComposerSelectorsAndSessions() throws {
        ...
        let contracts = [
            ...
            SourceContract(
                ...
                expected: "VoyagerDS.Shadow.popoverColor",
                forbidden: "Color.black.opacity(0.16)",
            ),
            SourceContract(
                file: "AiChatSelectors.swift",
                expected: "VoyagerDS.Interaction.controlHoverFill",
...
private extension CBW001ContextualChatRequestTests {
    ...
    func makeMalformedFinalUserFixture() -> CBW001MalformedFinalUserFixture {
        ...
        let store = TestStore(initialState: AiChatFeature.State(pendingRequestStart: pending)) {
            AiChatFeature()
        } withDependencies: {
            $0.uuid = .incrementing
