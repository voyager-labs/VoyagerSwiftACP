...
@Reducer
public struct UnlockAccessFeature {
    public typealias State = UnlockAccessState
    public typealias Action = UnlockAccessAction
    ...
    private enum CancelID {
        static let claim = "unlockAccessClaim"
        static let fetchStatus = "unlockAccessFetchStatus"
    }
    ...
    public var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case let .accessStatusResponse(result):
                return handleAccessStatusResponse(&state, result: result)
    ...
    private func handleOnAppear(_ state: inout State) -> Effect<Action> {
        ...
        return .run { [accessClient] send in
            ...
            await send(.accessStatusResponse(result))
        }
    ...
    private func handleSubmit(_ state: inout State) -> Effect<Action> {
        ...
        return .run { [accessClient] send in
            let result: Result<AccessStatusResponse, AccessError>
            do {
                let response: AccessStatusResponse = switch mode {
                case .licenseKey:
                    try await accessClient.claimLicense(licenseKey)
                case .betaCode:
                    try await accessClient.redeemBetaCode(betaCode)
                }
                result = .success(response)
            } catch let error as AccessError {
                result = .failure(error)
