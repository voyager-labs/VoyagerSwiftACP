...
extension ACC002CheckEntitlementStatusTests {
    ...
    /// ACC-002-trusted-snapshot-fallback: 복원 완료를 소비할 때 만료된 admission은 unlock하지 않는다.
    func testExpiredFallbackAdmissionRequiresRecovery() async {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let binding = UUID()
        let sessionExpiry = now.addingTimeInterval(7200)
        let snapshot = AccessStatusSnapshot(
            status: .coreLicenseActive,
