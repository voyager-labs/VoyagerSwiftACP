...
public enum AccountTokenSessionMapper {
    /// AccountTokensFile → AccountSession 변환.
    /// accessToken이 빈 문자열이면 nil 반환.
    public static func tokensFileToSession(_ file: AccountTokensFile) -> AccountSession? {
        guard !file.accessToken.isEmpty else { return nil }
    ...
    public static func sessionToTokensFile(_ session: AccountSession) -> AccountTokensFile? {
