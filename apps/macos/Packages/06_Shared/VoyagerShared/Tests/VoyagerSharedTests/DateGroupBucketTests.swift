...
final class DateGroupBucketTests: XCTestCase {
    ...
    func testTodayReturnsToday() throws {
        let date = try dateAt(hour: 8, minute: 0)
    ...
    func testSameYearDifferentMonthReturnsMonth() throws {
        let date = try dateFromComponents(year: 2026, month: 2, day: 5)
    ...
    func testDifferentYearReturnsYear() throws {
        let date = try dateFromComponents(year: 2024, month: 11, day: 15)
    ...
    private func dateAt(hour: Int, minute: Int) throws -> Date {
        var comps = DateComponents()
        comps.year = 2026
        comps.month = 5
        comps.day = 10
        ...
        comps.minute = minute
        comps.calendar = calendar
        return try XCTUnwrap(comps.date)
    }

    private func dateFromComponents(year: Int, month: Int, day: Int) throws -> Date {
        var comps = DateComponents()
        comps.year = year
        comps.month = month
        comps.day = day
