...
struct ATI006CoordinateExternalAgentSessionsTests {
    ...
    /// ATI-006-coordinate_external_agent_launch: launch failure can retry the same run reference.
    /// receipt 전 adapter 오류를 launchFailed로 저장하고 같은 run의 후속 invocation을 허용하는지 검증한다.
    /// - 검증 내용: launch 오류의 원본 반환, launchFailed projection, 후속 provider 실행과 exact launch count.
    /// - 사전 조건: adapter launch가 첫 호출에서 receipt 없이 오류를 반환한다.
    /// - 기대 결과: 첫 호출은 adapterUnavailable이고 두 번째 호출은 같은 run을 한 번만 재시작한다.
    @Test
    func `launch failure can retry the same run reference`() async throws {
        let adapter = DeterministicRuntimeAdapter(
            id: "sdk",
            transport: .sdkAsyncStream,
            eventsByLaunch: [[]],
        ...
        #expect(await plane.projection(for: request.externalAgentSessionReference) == .launchFailed)
        #expect(await adapter.counts().launch == 1)
        #expect(try await runPolicyReady(plane, request).outcome == .completed)
        #expect(await plane.projection(for: request.externalAgentSessionReference) == .completed)
        #expect(await adapter.counts().launch == 2)
    }

    /// ATI-006-coordinate_external_agent_launch: cancelling launch wait persists launch cancellation.
    /// receipt 전 caller task 취소를 launchCancelled로 저장하고 provider 재호출을 차단하는지 검증한다.
    /// - 검증 내용: CancellationError 전파, launchCancelled projection, provider 재호출 차단.
    /// - 사전 조건: adapter launch가 receipt 반환 전 cancellation-aware delay에서 대기한다.
    /// - 기대 결과: launchCancelled reservation은 fail-closed로 남고 같은 run은 duplicate로 거부된다.
    @Test
    func `cancelled launch wait persists launch cancellation`() async throws {
    ...
    /// ATI-006-coordinate_external_agent_run_continuity: restored terminal event wins over terminal-result failure.
    /// 복원된 run이 terminal event를 저장한 뒤 provider terminal-result 조회가 실패해도 durable terminal로 수렴하는지 검증한다.
    /// - 검증 내용: resume 반환 outcome, completed projection, provider stream과 terminal-result 호출 횟수.
    /// - 사전 조건: running snapshot을 복원하고 completed event 저장 뒤 terminal-result adapter 오류가 발생한다.
    /// - 기대 결과: fresh-run 오류 규칙과 분리된 resume 경계가 같은 run의 committed completed 결과를 반환한다.
    @Test
    func `restored terminal event wins over terminal-result failure`() async throws {
        let host = ExternalAgentSessionReference("host-resume-terminal-precedence")
        let run = RuntimeRunReference("run-resume-terminal-precedence")
        let context = finalReviewTestsMakeContext()
        let stored = RuntimeStoredSession(
            externalAgentSessionReference: host,
            providerInternalSessionReference: ProviderInternalSessionReference("opaque-resume-terminal"),
            runReference: run,
            adapterID: RuntimeAdapterID("sdk"),
            adapterVersion: "1.0.0",
            capabilitySnapshot: .allSupported,
        ...
        let adapter = DeterministicRuntimeAdapter(
            ...
            eventsByLaunch: [[makeEvent(
                ...
                run: run,
                sequence: 1,
                idempotencyKey: "restored-terminal-precedence",
                kind: .completed,
            )]],
        ...
        #expect(resumed.runReference == run)
    ...
    func `restored terminal probe failure releases its claim`(
