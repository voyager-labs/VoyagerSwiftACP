...
@Reducer
public struct ComposerSearchLifecycleReducer: Sendable {
        ...
        Reduce { state, action in
            ...
            case let .internal(.searchResponse(requestID, response)):
                ...
                case let .success(response):
                    ...
                    let capturedSearchClient = searchClient
                    return .run { send in
                        do {
                            let executionResponse = try await capturedSearchClient.applyFilters(
