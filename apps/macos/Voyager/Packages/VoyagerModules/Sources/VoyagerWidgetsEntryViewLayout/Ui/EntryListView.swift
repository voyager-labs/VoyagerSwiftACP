...
@MainActor
public struct EntryListViewRepresentable<ContentStore>: NSViewRepresentable {
    ...
    public func makeNSView(context _: Context) -> EntryListView {
    ...
    public func updateNSView(_ view: EntryListView, context _: Context) {
        if let coordinator = view.coordinator {
            coordinator.updateRootView(view)
        } else {
            let coordinator = EntryListCoordinator(store: store)
            view.coordinator = coordinator
            coordinator.bind(to: view)
...
public final class EntryListView: NSView {
    var coordinator: EntryListCoordinator?

    @MainActor
    public protocol EntryListTableViewContextMenuProviding: AnyObject {
        func contextMenu(forRow row: Int?, event: NSEvent) -> NSMenu
    }

    public final class EntryListTableView: NSOutlineView {
        public weak var contextMenuProvider: EntryListTableViewContextMenuProviding?
    ...
    public let tableView = EntryListTableView()
    ...
    public func applyColumns(_ visibleColumns: [EntryListColumn]) {
