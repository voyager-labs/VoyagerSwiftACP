...
extension EntryGridCoordinator {
    ...
    func observeScrollForThumbnailPrefetch() {
        ...
        let throttler = thumbnailPrefetchThrottler
    ...
    func refreshThumbnailProjection(paths: Set<String>) -> Set<String> {
        ...
        for path in paths {
            let cachedImage = entryThumbnailCacheClient.getThumbnail(path)
            if let cachedImage {
    ...
    func refreshThumbnailProjectionForVisibleArea() {
        ...
        let rawAttrs = layout.layoutAttributesForElements(in: visibleRect)
        let indexPaths: [IndexPath] = rawAttrs.compactMap { attr in
            guard attr.representedElementCategory == .item else { return nil }
            return attr.indexPath
        }
        guard !indexPaths.isEmpty else { return }
