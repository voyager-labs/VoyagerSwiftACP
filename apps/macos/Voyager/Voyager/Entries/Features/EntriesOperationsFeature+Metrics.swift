...
extension EntriesOperationsFeature {
    struct EntryActionPayload {
        let actionKind: DAUEntryActionKind
        let entryKind: DAUEntryKind
    }

    func logDAUEntryActionIfNeeded(_ action: Action) {
    ...
    func dauEntryActionPayload(for action: Action) -> EntryActionPayload? {
    ...
    func entryKind(for entries: [Entry]) -> DAUEntryKind? {
    ...
    func entryKind(for entry: Entry) -> DAUEntryKind {
    ...
    func entryKind(for targets: [TagChangeTarget]) -> DAUEntryKind? {
        entryKind(for: targets.map(\.file))
    }

    func entryKind(for paths: [String]) -> DAUEntryKind? {
    ...
    func entryKind(forPath path: String) -> DAUEntryKind {
    ...
    func dauActionKind(for actionKind: EntryActionRecord.ActionKind) -> DAUEntryActionKind {
        switch actionKind {
        case .rename:
            .rename
        case .move:
