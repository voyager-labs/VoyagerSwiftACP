...
struct ContentPageView: View {
    ...
    var body: some View {
        mainContent
            ...
            .onChange(of: store.entryViewLayout.isRenaming) { isRenaming in
                guard isGridLayout else { return }
                keyCommandFocusCoordinator?.setFocusAllowed(!isRenaming)
                if !isRenaming {
            ...
            .onAppear {
                guard isGridLayout else { return }
                keyCommandFocusCoordinator?.setFocusAllowed(!store.entryViewLayout.isRenaming)
    ...
    private func restoreKeyCommandFocus() {
        guard !store.entryViewLayout.isRenaming else { return }
    ...
    private static func makeEntryViewLayoutPageState(from state: FileManagerContentFeature.State) -> EntryViewLayoutAdapter.PageState {
        .init(
