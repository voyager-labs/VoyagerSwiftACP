...
private extension EntryListTableViewController {
    func addOpenItems(to menu: NSMenu, rowEntry: Entry?, selectedEntries: [Entry]) {
        ...
        if let openWithMenu = buildOpenWithMenu(rowEntry: rowEntry, selectedEntries: selectedEntries) {
    ...
    func buildOpenWithMenu(rowEntry _: Entry?, selectedEntries: [Entry]) -> NSMenu? {
        ...
        if !applications.isEmpty {
            ...
            for app in applications {
                let title = app.name
                let appItem = NSMenuItem(title: title, action: #selector(contextMenuOpenWithApp(_:)), keyEquivalent: "")
