...
| ----------------------------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| Package build                                         | `xcrun swift build --package-path <path>`                     | Local SwiftPM package compilation                                           | —                                                                        | Package path or toolchain cannot be identified                                        |
| Package focused suite                               | `xcrun swift test --package-path <path> --filter <suite>`     | One named package test target and suite                                    | No broader run substitutes for an unidentified focused suite             | Package path, target, or suite cannot be identified                                  |
| Package full test                                     | `xcrun swift test --package-path <path>`                      | All tests owned by one SwiftPM package                                      | —                                                                        | Package path or toolchain cannot be identified                                        |
...
| macOS scheme build                                    | `mise run macos-build`                                         | Voyager macOS development scheme                                             | Stop if the repository task cannot run                                  | The repository task is unavailable or cannot prove the required scheme                |
| macOS mapped flow                                     | `mise run macos-test-flow -- --flow <flow-id>`                  | One canonical mapped `VoyagerTests` flow                                    | `mise run macos-test` only for explicitly broad app coverage            | Flow ID or mapping cannot be identified                                               |
| macOS app focused suite                               | `mise run macos-test -- -only-testing:VoyagerTests/<suite>`     | One named general `VoyagerTests` suite                                      | `mise run macos-test` only for explicitly broad app coverage            | Suite identity cannot be identified                                                   |
| macOS scheme full test                                | `mise run macos-test`                                          | Voyager macOS development scheme; inspect its result/log for target coverage | Stop if the repository task cannot run                                  | The repository task is unavailable or cannot prove intended targets                   |
...
Route selection is owner-scoped. A package receipt proves only the selected SwiftPM package suite; an app receipt proves only the selected app route. App test-plan membership, build-for-testing, and dry-run output do not prove package test execution or a positive executed test count.
