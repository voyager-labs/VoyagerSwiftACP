...
One spec = one owning test suite named `<SpecID><PascalCaseSpecTitle>Tests.swift`. Interaction ACs = `// MARK:` sections inside that suite. Infrastructure = `Support/`. No `FeatureTests` suffix. Focused filters use the suite class name, not the test target name. Hand off to `testing` skill for execution.
...
7. Put fixtures, recorders, dependency doubles, builders, and helper assertions under `Support/`; support files must not own product behavior.
8. After authoring, hand off to `../testing/SKILL.md` for focused command selection, execution, failure analysis, and reruns; the initial filter should target the suite class such as `--filter ONB004FinishOnboardingTests`, not `--filter VoyagerPagesOnboardingTests`.
