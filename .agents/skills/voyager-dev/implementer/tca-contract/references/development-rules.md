...
- In `@MainActor` static builders (e.g. menu builders, projection helpers), declare `@Dependency(\.x)` as a LOCAL variable inside the function, not as a static property. A static `@Dependency` is lazily captured and unsafe for `withDependencies` test overrides; a local `@Dependency` reads `DependencyValues._current` at access time and honors overrides. See `EntryContextMenuBuilder.openWithIcon(for:)` for the canonical pattern.
...
- For URL-like dependencies, prefer canonical base env keys plus code-owned route paths. Use destination-specific URL keys only when product/app contracts define them, and never use neutral invalid fallbacks or hardcoded production URLs in live values.
