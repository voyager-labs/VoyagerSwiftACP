...
# Voyager Dev Spec Test Authoring (Implementer)

## Core rule

Test design starts by selecting exactly one owner: a product spec interaction or a canonical flow. One spec = one owning test suite named `<SpecID><PascalCaseSpecTitle>Tests.swift`, physically contained in that canonical file. Do not split a spec-owner suite into `Suite+Subtopic.swift` extension files; merge existing extension methods into the canonical suite and keep only fixtures/doubles/helpers under `Support/`. Interaction ACs = `// MARK:` sections inside that suite. Every interaction test method has a `///` traceability doc comment with scenario, verification content, preconditions, and expected result. Infrastructure = `Support/`. No `FeatureTests` suffix. Focused filters use the suite class name, not the test target name. Canonical flow documents own production-composition suites under `VoyagerTests/Flows/`; spec-owner suites own interaction-level AC detail. Without a spec or flow owner, do not create an automated test file: add the owner document first. Hand off to `testing` skill for execution.
...
9. For entry-manipulation, entry collection, `.voycoll`, or entry path-display specs, apply `references/entry-fixture-source.md` as the canonical fixture contract.
10. After authoring, load `references/testing-playbook.md` for focused command selection, execution, failure analysis, and reruns; the initial filter should target the suite class such as `--filter <SpecID><PascalCaseSpecTitle>Tests`, not the test target name.
...
12. Never create standalone component, infrastructure, contract, or UI test files (e.g. `AccountTokenFileStoreTests.swift`, `EnvironmentLoaderTests.swift`) as ad-hoc proof for an implementation change. `Specs/` contains ONLY spec-owner suites and `Flows/` contains ONLY canonical flow suites. If implementation behavior needs coverage but no spec AC or canonical flow exists, update the owner document first, then add test methods to its suite under the appropriate `// MARK:` section. If the user did not ask for test authoring, stop at existing-test/build evidence and report the proof gap. See `references/spec-test-topology.md` section "Specs/ directory contents" and "Component behavior coverage workflow".
...
7. **Fixture boundary**: Delegate fixture source, sandboxing, path-equivalence, and OS-boundary mock requirements to `references/entry-fixture-source.md`; this entry point retains only the requirement to load that contract.
