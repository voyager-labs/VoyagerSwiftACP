---
globs: apps/macos/Voyager/Voyager/**
description: "Always load voyager-dev for Voyager app work."
---
...
- Editing files under `apps/macos/Voyager/Voyager/**`.

## Must

- Load `.agents/skills/voyager-dev/SKILL.md` before planning or implementing changes in this path.
- Use `voyager-dev` as the default companion workflow even for medium-sized refactors in this app target.
...
- Skip `voyager-dev` for non-trivial work in `apps/macos/Voyager/Voyager/**`.
- Add ad-hoc TCA/FSD structure changes in this path without checking the relevant `voyager-dev` guidance first.

## Execution steps

1. Load `.agents/skills/voyager-dev/SKILL.md` at task start.
