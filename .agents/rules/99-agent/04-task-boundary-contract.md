...
- Record the task outcome when work finishes (pass, fail, degraded, or exception, per `99-agent/06-outcome-classification.md`).
...
3. After work, run verification matching the intent (see `99-agent/05-verification-intent-parity.md`).
4. Classify the outcome using the taxonomy in `99-agent/06-outcome-classification.md`.
...
- Treat partial completion as pass. Use degraded or exception classification from `99-agent/06-outcome-classification.md`.
