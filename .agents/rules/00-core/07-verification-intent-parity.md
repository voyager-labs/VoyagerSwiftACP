...
- Running verification after code or artifact changes.
- Designing verification steps for a task plan.
- Reviewing whether completed work was adequately verified.
...
- When intent has multiple parts, verify each part independently. Partial verification yields a degraded outcome (per `08-outcome-classification.md`).
- Use the scope-based check matrix in `02-verification.md` for required checks, then add intent-specific checks on top.
...
1. Read the task intent statement (from `06-task-boundary-contract.md`).
2. List what "done" means for that intent specifically.
3. Run the scope-required checks from `02-verification.md`.
...
6. Classify the overall verification outcome using `08-outcome-classification.md`.
...
- Verification outcome matches the classification in `08-outcome-classification.md`.
