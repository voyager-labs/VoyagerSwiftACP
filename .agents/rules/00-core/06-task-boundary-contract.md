...
- Any work session that modifies files, produces artifacts, or makes decisions.
- Multi-step plans broken into sequential tasks.
...
- Record the task outcome when work finishes (pass, fail, degraded, or exception, per `08-outcome-classification.md`).
...
- Treat partial completion as pass. Use degraded or exception classification from `08-outcome-classification.md`.
...
3. After work: run verification matching the intent (see `07-verification-intent-parity.md`).
4. Classify the outcome using the taxonomy in `08-outcome-classification.md`.
