...
- [ ] <<AI>> 설정 창의 General 탭이 표시된 상태일 때, 상태 점검을 실행하면, Full Disk Access 및 Notifications의 권한 상태가 최신 값으로 갱신됨.
- [ ] <<AI>> 사용자가 시스템 설정에서 권한을 변경하고 앱으로 돌아온 상태일 때, 상태 점검을 실행하면, 변경된 권한 상태가 설정 화면에 반영됨.
- [ ] <<AI>> 상태 조회가 실패한 상태일 때, 상태 점검을 실행하면, 기존 표시를 유지하고 재시도 안내가 표시됨.
- [ ] <<AI>> 상태 반영이 지연되는 상태일 때, 상태 점검을 실행하면, 지연 안내가 표시되고 재점검 트리거가 유지됨.
