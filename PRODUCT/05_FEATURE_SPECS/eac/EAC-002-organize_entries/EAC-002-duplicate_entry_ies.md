---
interaction_id: "EAC-002-duplicate_entry_ies"
...
- [ ] <<AI>> 복제 가능한 상태일 때, 사용자가 해당 인터랙션을 호출하면, 선택한 Entry의 복제본을 동일
      위치에 생성함.
- [ ] <<AI>> 이름 충돌이 발생하는 상태일 때, 시스템이 복제본을 생성하면, 충돌을 회피하는 이름
      규칙으로 복제본 이름을 결정함.
- [ ] <<AI>> 복제 불가한 상태일 때, 사용자가 해당 인터랙션을 호출하면, 복제를 수행하지 않고 실패
      사유를 사용자에게 안내함.
