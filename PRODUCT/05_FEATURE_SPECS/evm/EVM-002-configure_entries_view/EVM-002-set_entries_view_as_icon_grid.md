...
- 그리드 항목의 태그 색상 규칙은 EntryArrangements를 따른다.
- 그리드의 아이콘 크기와 텍스트 크기는 사용자 설정값(해당 뷰의 값)을 사용한다.
...
- [ ] Entry에 태그가 있는 상황에서, 그리드 항목의 태그 색상 규칙은 EntryArrangements를 따라야 한다.

## Permissions / Dependencies

- Entries 목록 데이터가 로드되어 있어야 한다.
- 그룹 헤더 및 태그 색상 규칙을 위해 EntryArrangements 정보를 참조할 수 있어야 한다.
