---
interaction_id: "EIX-009-extract_image_properties"
...
- [ ] <<AI>> 대상 Entry가 이미지 포맷인 상태일 때, 시스템이 추출을 수행하면, UI 캡처
      여부·그래프/차트 여부·캡션/설명 등 자동 프로퍼티를 저장함.
- [ ] <<AI>> 추출이 완료된 상태일 때, 사용자가 해당 Entry를 검색하면, 추출된 프로퍼티 기반 조건으로
      필터링할 수 있게 함.
- [ ] <<AI>> 추출이 실패한 상태일 때, 시스템이 실패를 기록하면, 실패 항목을 오류 상세에서 확인
      가능하게 함.
