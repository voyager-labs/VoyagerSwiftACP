---
interaction_id: "EOP-004-batch_rename_entries"
...
- `file_menu` 메뉴의 Batch Rename Entries 항목
...
- 선택한 여러 Entry의 이름에 패턴·번호 매김·문자열 치환 규칙을 적용해 일괄적으로 변경.
- 사용자에게 보이는 결과는 `batch_rename_entries_applied` 상태로 정리된다.

## State Changes

- Entry selection의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [eop_contract.toml](../contracts/eop_contract.toml)의 `batch_rename_entries_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entry selection이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Batch Rename Entries을 실행하면, 선택한 여러 Entry의 이름에 패턴·번호 매김·문자열 치환 규칙을 적용해 일괄적으로 변경 결과가 `batch_rename_entries_applied` 상태로 반영되어야 한다.
...
- `eop.batch_rename_entries` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- [EOP-004-rename_entry](EOP-004-rename_entry.md)

## Source

- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:63`
