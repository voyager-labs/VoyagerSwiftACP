...
- <<AI>> 하나 이상의 엔트리가 선택된 상태- 인스펙터 패인이 프로퍼티 모드이거나 커스텀 프로퍼티 편집
  UI에 접근 가능한 상태
...
- [ ] <<AI>> 하나 이상의 엔트리가 선택된 상태일 때, 사용자가 새 프로퍼티 키·타입·초기 값을 입력하고
      저장하면, 선택된 엔트리(들)에 커.스텀 프로퍼티를 추가하고 값을 저장함.
- [ ] <<AI>> 동일 키의 프로퍼티 스키마가 워크스페이스에 없는 상태일 때, 사용자가 저장하면,
      워크스페이스 차원의 프로퍼티 스키마를 생성하고 엔트리 프로퍼티가 해당 스키마를 참조하게 함.
- [ ] <<AI>> 키 또는 초기 값 입력이 유효하지 않은 상태일 때, 사용자가 저장하면, 저장을 차단하고
      유효성 오류를 표시함.
