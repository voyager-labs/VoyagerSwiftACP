...
- 빌드만 확인: `mise run macos-launch -- --scheme Voyager-Dev --configuration Dev-Debug --no-launch`
    ...
    - VSCode 류 IDE(Sweetpad Extension)에서 실행:
        ...
        - 태스크로 실행 (권장):
            ...
            - `Voyager Dev: Launch (Dev-Debug)` / `Voyager Dev: Launch (Dev-Release)` / `Voyager Prod: Launch (Prod-Debug)` / `Voyager Prod: Launch (Prod-Release)`
                ...
                - `ComposerHost Dev: Launch (Debug)` - Composer 전용 deterministic 호스트를 Debug 모드로 실행
                - `ComposerHost Dev: Launch (Release)` - Composer 전용 deterministic 호스트를 Release 모드로 실행
    ...
    - Zed 에디터에서 실행 (`.zed/tasks.json`):
        ...
        - 사용 가능한 태스크 (VSCode Sweetpad와 동일):
            ...
            - `ComposerHost Dev: Launch (Debug)` / `ComposerHost Dev: Launch (Release)`
    ...
    - ComposerHost Launch (CLI): `mise run macos-launch -- --scheme ComposerHost-Dev --configuration Debug`
    - ComposerHost Smoke (CLI): `mise run macos-launch -- --scheme ComposerHost-Dev --configuration Debug --env COMPOSER_HOST_SMOKE=1`
