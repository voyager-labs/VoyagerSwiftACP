...
`docs/`는 세 가지 문서 레인을 가진다.

- `canonical/`: 문서 submodule이 제공하는 제품, 비즈니스, 웹사이트, 그로스 SSOT.
- `engineering/`: 이 앱 레포가 소유하는 active engineering 지식 베이스.
- `legacy/`: 재구성 이전 앱 문서를 감사/이관 전까지 보관하는 reference-only 영역.

## Rules

- 앱 레포 engineering 문서 작업에서는 작업이 명시적으로 문서 submodule을 대상으로 하지 않는 한 `docs/canonical/**`을 수정하지 않는다.
- `docs/legacy/**`는 기본적으로 읽기 전용 reference로 취급한다. 레거시 페이지를 확장하기보다 유효한 내용을 `docs/engineering/**`로 이관한다.
- 새로운 기술 결정, 런타임 흐름, 구현 노트, 코드/테스트 topology 문서는 `docs/engineering/**`에 둔다.
