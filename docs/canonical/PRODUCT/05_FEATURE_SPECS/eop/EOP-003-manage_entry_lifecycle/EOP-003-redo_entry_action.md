---
interaction_id: "EOP-003-redo_entry_action"
...
- `edit_menu` 메뉴의 Redo Entry Action 항목
- `⇧⌘Z` 단축키
...
- Undo로 되돌린 최근 Entry 관련 액션을 다시 적용.
- 사용자에게 보이는 결과는 `redo_entry_action_applied` 상태로 정리된다.

## State Changes

- Entry의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [eop_contract.toml](../contracts/eop_contract.toml)의 `redo_entry_action_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entry이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Redo Entry Action을 실행하면, Undo로 되돌린 최근 Entry 관련 액션을 다시 적용 결과가 `redo_entry_action_applied` 상태로 반영되어야 한다.
...
- `eop.redo_entry_action` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:58`
