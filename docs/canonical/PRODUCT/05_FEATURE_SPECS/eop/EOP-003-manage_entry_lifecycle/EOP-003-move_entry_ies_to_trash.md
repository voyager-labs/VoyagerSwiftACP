---
interaction_id: "EOP-003-move_entry_ies_to_trash"
...
- `file_menu` 메뉴의 Move Entry(ies) to Trash 항목
- `⌘⌫` 단축키
...
- 선택한 Entry를 휴지통으로 이동.
- 사용자에게 보이는 결과는 `move_entry_ies_to_trash_applied` 상태로 정리된다.

## State Changes

- Entry selection의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [eop_contract.toml](../contracts/eop_contract.toml)의 `move_entry_ies_to_trash_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entry selection이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Move Entry(ies) to Trash을 실행하면, 선택한 Entry를 휴지통으로 이동 결과가 `move_entry_ies_to_trash_applied` 상태로 반영되어야 한다.
...
- `eop.move_entry_ies_to_trash` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:53`
