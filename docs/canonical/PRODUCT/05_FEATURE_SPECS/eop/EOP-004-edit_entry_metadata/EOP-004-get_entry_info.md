---
interaction_id: "EOP-004-get_entry_info"
...
- `file_menu` 메뉴의 Get Entry Info 항목
- `⌘I` 단축키
...
- 선택한 Entry의 상세 정보 패널을 표시.
- 사용자에게 보이는 결과는 `get_entry_info_applied` 상태로 정리된다.

## State Changes

- Entry의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [eop_contract.toml](../contracts/eop_contract.toml)의 `get_entry_info_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entry이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Get Entry Info을 실행하면, 선택한 Entry의 상세 정보 패널을 표시 결과가 `get_entry_info_applied` 상태로 반영되어야 한다.
...
- `eop.get_entry_info` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:61`
