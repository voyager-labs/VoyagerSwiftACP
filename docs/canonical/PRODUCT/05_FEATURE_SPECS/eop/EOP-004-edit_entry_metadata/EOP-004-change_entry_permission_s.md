---
interaction_id: "EOP-004-change_entry_permission_s"
...
- `file_menu` 메뉴의 Change Entry Permission(s) 항목
...
- 선택한 Entry의 읽기·쓰기·실행 권한과 소유자/그룹을 수정.
- 사용자에게 보이는 결과는 `change_entry_permission_s_applied` 상태로 정리된다.

## State Changes

- Entry의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [eop_contract.toml](../contracts/eop_contract.toml)의 `change_entry_permission_s_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entry이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Change Entry Permission(s)을 실행하면, 선택한 Entry의 읽기·쓰기·실행 권한과 소유자/그룹을 수정 결과가 `change_entry_permission_s_applied` 상태로 반영되어야 한다.
...
- `eop.change_entry_permission_s` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:62`
