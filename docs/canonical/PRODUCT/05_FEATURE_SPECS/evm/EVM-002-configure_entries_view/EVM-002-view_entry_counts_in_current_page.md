...
- 표시할 Entries View 상태가 현재 Page 또는 selection에서 계산 가능한 상태다.

## Expected Outcome

- 현재 Page에 존재하는 Entry의 총 개수를 표시.
- 사용자에게 보이는 결과는 `view_entry_counts_in_current_page_applied` 상태로 정리된다.

## State Changes

- Entries View의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [evm_contract.toml](../contracts/evm_contract.toml)의 `view_entry_counts_in_current_page_applied` 상태 어휘를 따른다.
...
- [ ] 표시할 Entries View 상태가 현재 Page 또는 selection에서 계산 가능한 상태다. 사용자가 View Entry Counts in Current Page을 실행하면, 현재 Page에 존재하는 Entry의 총 개수를 표시 결과가 `view_entry_counts_in_current_page_applied` 상태로 반영되어야 한다.
...
- `evm.view_entry_counts_in_current_page` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:32`
