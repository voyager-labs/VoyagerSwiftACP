...
feature_id: "EVM-002"
status: "배포 완료"
summary: "Entries View를 아이콘 레이아웃으로 전환해 동일한 Entry 데이터를 시각적으로 표시"
...
- `view_menu` 메뉴의 Set Entries View as Icon 항목
...
- Entries View를 아이콘 레이아웃으로 전환해 동일한 Entry 데이터를 시각적으로 표시.
- 사용자에게 보이는 결과는 `set_entries_view_as_icon_grid_applied` 상태로 정리된다.

## State Changes

- Entries View의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [evm_contract.toml](../contracts/evm_contract.toml)의 `set_entries_view_as_icon_grid_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entries View이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Set Entries View as Icon을 실행하면, Entries View를 아이콘 레이아웃으로 전환해 동일한 Entry 데이터를 시각적으로 표시 결과가 `set_entries_view_as_icon_grid_applied` 상태로 반영되어야 한다.
...
- `evm.set_entries_view_as_icon_grid` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:25`
