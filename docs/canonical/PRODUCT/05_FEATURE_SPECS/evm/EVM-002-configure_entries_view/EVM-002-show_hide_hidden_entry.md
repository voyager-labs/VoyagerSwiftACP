...
- `view_menu` 메뉴의 Show/Hide Hidden Entry 항목
- `⌘⇧.` 단축키
...
- Entries View에서 숨김 설정된 Entry의 Entry 목록 상에 보이게 하거나, 보이지 않도록 토글.
- 사용자에게 보이는 결과는 `show_hide_hidden_entry_applied` 상태로 정리된다.

## State Changes

- Entries View의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [evm_contract.toml](../contracts/evm_contract.toml)의 `show_hide_hidden_entry_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Entries View이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Show/Hide Hidden Entry을 실행하면, Entries View에서 숨김 설정된 Entry의 Entry 목록 상에 보이게 하거나, 보이지 않도록 토글 결과가 `show_hide_hidden_entry_applied` 상태로 반영되어야 한다.
...
- `evm.show_hide_hidden_entry` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:30`
