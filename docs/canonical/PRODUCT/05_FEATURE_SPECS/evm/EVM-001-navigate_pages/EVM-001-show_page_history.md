...
- 현재 Content Tab의 페이지 히스토리를 드랍다운 리스트로 표시.
- 사용자에게 보이는 결과는 `show_page_history_applied` 상태로 정리된다.

## State Changes

- Page의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [evm_contract.toml](../contracts/evm_contract.toml)의 `show_page_history_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Page이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Show Page History을 실행하면, 현재 Content Tab의 페이지 히스토리를 드랍다운 리스트로 표시 결과가 `show_page_history_applied` 상태로 반영되어야 한다.
...
- `evm.show_page_history` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:20`
