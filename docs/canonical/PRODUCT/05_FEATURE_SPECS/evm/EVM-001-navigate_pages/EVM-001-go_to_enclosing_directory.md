---
interaction_id: "EVM-001-go_to_enclosing_directory"
...
- 현재 Page가 표시하는 디렉토리의 상위 디렉토리로 이동.
- 사용자에게 보이는 결과는 `go_to_enclosing_directory_applied` 상태로 정리된다.

## State Changes

- Page의 표시 또는 실행 상태를 갱신한다.
- 이 인터랙션은 [evm_contract.toml](../contracts/evm_contract.toml)의 `go_to_enclosing_directory_applied` 상태 어휘를 따른다.
...
- [ ] 대상 Page이 현재 File Manager Window에서 접근 가능한 상태다. 사용자가 Go to Enclosing Directory을 실행하면, 현재 Page가 표시하는 디렉토리의 상위 디렉토리로 이동 결과가 `go_to_enclosing_directory_applied` 상태로 반영되어야 한다.
...
- `evm.go_to_enclosing_directory` 이벤트에 성공 여부와 대상 수, 실패 사유를 기록한다.
...
- Inventory row: `PRODUCT/04_FEATURE_INVENTORY/INTERACTIONS/data.tsv:21`
