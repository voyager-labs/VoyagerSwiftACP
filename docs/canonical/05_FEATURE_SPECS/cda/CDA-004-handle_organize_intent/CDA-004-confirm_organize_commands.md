...
- [ ] <<AI>> Organize Commands 프리뷰가 표시된 상태일 때, 사용자가 Confirm Organize Commands 인터랙션에서 실행을 승인하면, 승인된 명령 목록이 Execute Organize Commands 단계로 한 번만 전달됨.
- [ ] <<AI>> Organize Commands 프리뷰가 표시된 상태일 때, 사용자가 Confirm Organize Commands 인터랙션에서 실행을 거절하면, 어떤 명령도 실행되지 않고 프리뷰가 닫힘.
- [ ] <<AI>> 같은 User Request에 대해 사용자가 Confirm Organize Commands 인터랙션을 반복 호출한 상태일 때, 시스템이 후속 호출을 처리하면, 첫 승인 또는 거절 결과만 유효하게 유지되고 중복 요청은 무시됨.
