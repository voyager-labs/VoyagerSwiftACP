...
- [ ] <<AI>> 하나 이상의 엔트리가 선택된 상태에서 사용자가 해당 인터랙션을 호출하면, 선택된 엔트리 집합이 현재 메시지의 컨텍스트로 추가되고 하나의 컨텍스트 칩으로 표시됨.
- [ ] <<AI>> 선택된 엔트리 중 일부에 접근할 수 없는 상태일 때, 사용자가 해당 인터랙션을 호출하면, 접근 가능한 엔트리만 컨텍스트로 포함되고 제외된 엔트리에 대해서는 제외 사유가 메타데이터로 기록되거나 사용자에게 안내됨.
- [ ] <<AI>> 선택된 엔트리가 없는 상태에서 사용자가 해당 인터랙션을 호출하면, 컨텍스트가 추가되지 않고 아무 변화도 일어나지 않음.
