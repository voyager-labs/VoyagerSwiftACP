...
- [ ] 현재 페이지가 필터 편집을 지원하는 상태일 때, 사용자가 해당 인터랙션을 호출하면, Collection Filter Composer가 표시되고 Filter Query 입력 필드로 포커스가 이동함
- [ ] Composer가 이미 열린 상태일 때, 사용자가 해당 인터랙션을 호출하면, Filter Query 입력 필드로 포커스가 이동함
- [ ] 미저장 필터 변경이 존재하는 상태일 때, 사용자가 해당 인터랙션을 호출하면, 미저장 변경이 반영된 Composer를 표시함
