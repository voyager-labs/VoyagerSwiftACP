...
- <<AI>> 사용자가 권한을 부여하지 않고 앱으로 돌아오는 경우- 권한을 부여했지만 상태 반영이 지연되는 경우
- <<AI>> 권한을 부여했지만 앱 재시작이 필요한 경우
- <<AI>> MDM/보안 정책 등으로 권한 변경이 제한되는 경우

## Acceptance Criteria

- [ ] <<AI>> 설정 창의 General 탭이 표시된 상태일 때, 사용자가 해당 인터랙션을 호출하면, Full Disk Access 필요 사유와 현재 권한 상태가 표시됨.
- [ ] <<AI>> 권한이 미허용인 상태일 때, 사용자가 “시스템 설정 열기”를 선택하면, 시스템 설정의 Full Disk Access 설정 화면으로 이동함.
- [ ] <<AI>> 직접 이동이 불가능한 상태일 때, 사용자가 “시스템 설정 열기”를 선택하면, 수동 이동 경로(Privacy & Security - Full Disk Access)가 안내됨.
- [ ] <<AI>> 사용자가 권한을 허용하고 앱으로 돌아온 상태일 때, 권한 상태를 재검증하면, 설정 화면의 상태가 “허용됨”으로 갱신됨.
- [ ] <<AI>> 사용자가 권한을 허용하지 않고 돌아온 상태일 때, 권한 상태를 재검증하면, 상태가 “미허용”으로 유지되고 재시도 안내가 표시됨.
