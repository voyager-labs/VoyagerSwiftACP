...
- [ ] <<AI>> 압축 파일이 선택된 상태일 때, 사용자가 해당 인터랙션을 호출하면, 현재 디렉토리 또는 지정한 위치로 압축을 해제함.
- [ ] <<AI>> 암호가 필요한 상태일 때, 시스템이 해제를 수행하면, 암호 입력을 요청하고 올바른 암호가 제공되면 해제를 진행함.
- [ ] <<AI>> 해제가 불가능하거나 부분 성공하는 상태일 때, 시스템이 해제를 수행하면, 성공/실패 결과와 사유를 사용자에게 안내함.
