...
- [ ] <<AI>> 하나 이상의 Entry가 선택된 상태일 때, 사용자가 해당 인터랙션을 호출하면, 태그를 추가·제거·변경할 수 있는 UI를 표시함.
- [ ] <<AI>> 태그 변경이 확정된 상태일 때, 시스템이 적용하면, 태그 메타데이터를 갱신하고 UI 표시를 반영함.
- [ ] <<AI>> 일부 Entry에 적용 불가한 상태일 때, 시스템이 적용하면, 적용 가능한 Entry만 반영하고 실패 대상과 사유를 사용자에게 안내함.
