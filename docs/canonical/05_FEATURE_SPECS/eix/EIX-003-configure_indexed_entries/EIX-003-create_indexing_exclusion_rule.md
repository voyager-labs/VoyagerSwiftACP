...
| Field | Value |
| --- | --- |
| Interaction ID | EIX-003-create_indexing_exclusion_rule |
...
- [ ] 제외 규칙 편집 화면이 표시된 상태일 때, 사용자가 규칙 생성을 저장하면, 시스템이 새 제외 규칙을 생성하고 규칙 목록에 반영함
- [ ] 입력한 경로·패턴이 유효하지 않은 경우일 때, 사용자가 저장하면, 시스템이 저장을 차단하고 유효하지 않은 입력을 표시함
- [ ] 새 규칙이 기존 규칙과 중복되는 경우일 때, 사용자가 저장하면, 시스템이 저장을 차단하거나 충돌을 해소하도록 안내함
