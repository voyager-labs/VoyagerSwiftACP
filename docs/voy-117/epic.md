...
#### Stories
1. **Story 1:** VOYAGER_PROJECT_ROOT 전달 및 source 모드 경로 탐지 안정화.
2. **Story 2:** 앱에서 Helper 생명주기 정책 구현(시작/감시/재시도/강제 종료 처리).
3. **Story 3:** Helper에서 backend 생명주기 관리 강화(실행/종료/재기동/포트 확보).
4. **Story 4:** UI 엔드포인트 탐지 로직 정리(`pgrep` + `lsof`) 및 runtime config 제거.
